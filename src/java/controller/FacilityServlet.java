...
public class FacilityServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int facHeadId = Integer.parseInt(request.getParameter("facHeadId").trim());
            
            Facilities fac = new Facilities();
          if(fac.addFacility(facName, detail, facHeadId) > 0){
            out.print("<script language = 'javascript'> alert('Facility successfuly added') </script>");
